...
for seedIdx in countup(0, seeds.len() - 1, 2):
  let (start, seedCount) = (seeds[seedIdx].parseInt(), seeds[seedIdx+1].parseInt())
  for seed in countup(start, start + seedCount - 1):
    seedToLocation.add(traverseMap(seed))
